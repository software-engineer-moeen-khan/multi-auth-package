...
namespace Moeen\LaravelMultiAuth\Commands;

use Illuminate\Support\Facades\Artisan;
use Illuminate\Support\Str;
use Symfony\Component\Console\Input\InputOption;
use Moeen\LaravelMultiAuth\Commands\InstallAndReplaceCommand;
use Moeen\LaravelMultiAuth\Commands\Traits\OverridesCanReplaceKeywords;
use Moeen\LaravelMultiAuth\Commands\Traits\OverridesGetArguments;
use Moeen\LaravelMultiAuth\Commands\Traits\ParsesServiceInput;
use SplFileInfo;
...
class MultiAuthInstallCommand extends InstallAndReplaceCommand
{
     ...
     * @var string
     */
    protected $name = 'laravel-multi-auth:install';
     ...
     * @var string
     */
    protected $description = 'Install Multi Auth into Laravel 6.* project';
    ...
    public function handle()
    {
        ...
        if ($this->option('force')) {
            ...
            Artisan::call('laravel-multi-auth:settings', [
                'name' => $name,
                'service' => $service,
                '--domain' => $domain,
                '--lucid' => $lucid,
            ...
            Artisan::call('laravel-multi-auth:files', [
                'name' => $name,
                'service' => $service,
                '--domain' => $domain,
                '--lucid' => $lucid,
            ...
            if (!$this->option('model')) {
                Artisan::call('laravel-multi-auth:model', [
                    'name' => $name,
                    '--lucid' => $lucid,
                    '--force' => true
                ]);
            ...
            if (!$this->option('views')) {
                Artisan::call('laravel-multi-auth:views', [
                    'name' => $name,
                    'service' => $service,
                    '--lucid' => $lucid,
                    '--force' => true
